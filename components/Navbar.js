...
function	Navbar({hasSecret}) {
	...
	return (
		<nav className={'w-full h-16 p-6 justify-center flex flex-row'}>
			<div className={'max-w-5xl items-center justify-between flex flex-row w-full'}>
				<div className={'flex flex-row items-center space-x-3'}>
					<Image src={'/yVempire.png'} width={42} height={42} quality={100} loading={'eager'} />
					<p className={`inline ${hasSecret ? 'text-white' : 'text-gray-800'} font-extrabold text-xl`}>{'yVempire'}</p>
				</div>
				<div>
					{renderWalletButton()}
				</div>
